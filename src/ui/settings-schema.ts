...
export function buildSettingEntries(submenus: {
  ...
  return [
    ...
    intEntry({
      id: "perAgentTokenLimit",
      name: "Per-agent token cap",
      get: (deps) => deps.manager.getPerAgentTokenLimit(),
      set: (deps, v) => deps.manager.setPerAgentTokenLimit(v),
      min: 0,
      max: 10_000_000,
      prompt: "Per-subagent token cap for spend warnings (0 = off)",
    }),

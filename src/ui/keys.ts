...
export function decodeKittyPrintable(data: string): string | undefined {
    ...
    if (!match)
        return undefined;
    // With Kitty flag 2, terminals emit press/repeat/release event types.
    // Only press events should produce printable input (release would double-insert).
    const eventType = parseEventType(match[5]);
    if (eventType === "release" || eventType === "repeat")
        return undefined;
